...
// 间距
const float BETTWEEN_LABEL_OFFSET = 80;
// 线宽  
const float LINE_WIDTH = 2.0;
// 圆的半径
const float CIRCLE_RADIUS = 13.0;
// 两端文字距流程图的距离
const float INITIAL_PROGRESS_CONTAINER_WIDTH = 20.0; // 20
// 距左边的距离
const float PROGRESS_VIEW_CONTAINER_LEFT = 30.0;  // 51
// ?
const float VIEW_WIDTH = 255.0;  // 255
...
- (void)addTimeDescriptionLabels:(NSArray *)timeDescriptions andTime:(NSArray *)time currentStatus:(int)currentStatus {
    ...
    for (NSString *timeDescription in timeDescriptions) {
        ...
        label.textColor = i < currentStatus - 1 ? [UIColor blueColor] : i == currentStatus - 1 ? [UIColor redColor] : [UIColor grayColor];
